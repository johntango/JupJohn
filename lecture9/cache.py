...
# Now let's motivate a further inefficiency in this representation.
...
def studentGradesInOrder(student):
    return sorted([(grade["pset"], grade["points"])
                   for grade in student["grades"].values()])

def psetGradesInOrder(pset):
    return sorted([(grade["points"], grade["student"])
                   for grade in pset["grades"].values()])
